			...
			<c:if test="<%= showPublicSite || showPrivateSite %>">
				<c:choose>
					<c:when test='<%= PropsValues.MY_SITES_DISPLAY_STYLE.equals("simple") %>'>
						...
						DynamicQuery visiblePublicLayoutCount = DynamicQueryFactoryUtil.forClass(Layout.class);
						visiblePublicLayoutCount.add(RestrictionsFactoryUtil.eq("groupId", mySiteGroup.getGroupId()));
						visiblePublicLayoutCount.add(RestrictionsFactoryUtil.eq("privateLayout", false));
						visiblePublicLayoutCount.add(RestrictionsFactoryUtil.eq("hidden", false));
						long visiblePublicLayoutCountResult = LayoutLocalServiceUtil.dynamicQueryCount(visiblePublicLayoutCount);
						%>

						<c:if test="<%= showPublicSite && ((/*mySiteGroup.getPublicLayoutsPageCount()*/ visiblePublicLayoutCountResult > 0) || showPublicSiteStaging) %>">
						...
						<c:if test="<%= showPrivateSite && ((mySiteGroup.getPrivateLayoutsPageCount() > 0) || showPrivateSiteStaging) %>">
							...
							<li class="<%= (selectedSite && layout.isPrivateLayout()) ? "active" : "private-site" %> <%= itemCssClass %>">
								<a href="<%= HtmlUtil.escape(portletURL.toString()) %>" onclick="Liferay.Util.forcePost(this); return false;">
									...
									if ((/*mySiteGroup.getPublicLayoutsPageCount()*/
											visiblePublicLayoutCountResult > 0) || showPublicSiteStaging) {
